...
public class Learning {
    ...
    GeneticPopulation optimizeGenerationParameters(FormulaPopulation pop, String[] variables, double[] ds2Times, double[][][] normal_model, double[][][] ineffective_model, double[][][] normal_model_test, double[][][] ineffective_model_test, double atTime) throws InterruptedException {
...
//            System.out.println(i+"/"+pop.getPopulationSize());
            ...
            if (!bestParameters.containsKey(keyFormula)) {
                bestParamterFormula = ComputeAverage.averageMultiTrajectory(100, function, variables, ds2Times, normal_model, ineffective_model, formula, pop, new double[]{GeneticOptions.min_time_bound, GeneticOptions.max_time_bound}, atTime);
...
//                    score = getScore(formula, pop, variables, ds2Times, normal_model, ineffective_model, normal_model_test, ineffective_model_test, atTime);
                    score = getScore2(formula, variables, ds2Times, normal_model_test, ineffective_model_test, bestParamterFormula, atTime);
    ...
    private double getScore2(Formula formula, String[] variables, double[] ds2Times, double[][][] normal_model_test, double[][][] ineffective_model_test, double[] bestFormulaParameters, double atTime) {
        double[] positiveSetPerformance = ComputeAverage.smcMultiTrajectories(ds2Times, normal_model_test, variables, formula, bestFormulaParameters, atTime);
        double[] negativeSetPerformance = ComputeAverage.smcMultiTrajectories(ds2Times, ineffective_model_test, variables, formula, bestFormulaParameters, atTime);
        return positiveSetPerformance[0] - negativeSetPerformance[0];
    }
}
