...
public class AllFormulas {
    ...
    private void getTrajectory() throws IOException, ParseException {
        String location = "/home/ssilvetti/git/eggloop/scripts/traccia.json";
        ...
        double[] space = IntStream.range(0, 20).mapToDouble(s -> 0.5 / 20. * s).toArray();
...
//        double[] uniqueSpace = Arrays.stream(array).distinct().toArray(String[]::new);
        ...
        Function<double[], Boolean> eval = value -> prova.accept(booleanTemporalMonitoring).evaluate(ass.apply(value));
