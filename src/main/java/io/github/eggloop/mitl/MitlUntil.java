...
public final class MitlUntil extends MiTL {
    ...
    public double evaluateValue(Trajectory x, double t) { //FIXME : need to be reviewd.
        ...
        index1 = timeIndexAfter(times, t1);
        index2 = timeIndexUntil(times, t2);
        if (index1 > index2 || t1 == t2) {
            return Math.max(formula1.evaluateValue(x, times[index2]), formula2.evaluateValue(x, times[index2]));
        }
    ...
    @Override
    public MiTL clone() {
        return new MitlUntil(formula1.clone(), t1, t2, formula2.clone());
    }
