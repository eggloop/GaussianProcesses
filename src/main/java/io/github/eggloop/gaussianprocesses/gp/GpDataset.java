...
final public class GpDataset {
	...
	private boolean canUsedCachedResults(final Kernel func) {
		return cachedKernel == func.getClass()
				&& Arrays.equals(cachedHyperparams, func.getHypeparameters());
	}

	public final double[] calculateVariances(final Kernel func) {
		final int n = this.getSize();
		final double[][] x = this.dataX;
		final double[] vars = new double[n];
		for (int i = 0; i < n; i++)
			vars[i] = func.distance(x[i], x[i]);
		return vars;
	}

	public final double[][] calculateCovariances(final Kernel func) {
		...
		if (canUsedCachedResults(func)) {
			...
			for (int i = 0; i < n; i++) {
				if (i < m)
					...
					result[i][i] = func.distance(x[i], x[i]);
				for (int j = i + 1; j < n; j++)
					if (i < m && j < m)
			...
			for (int i = 0; i < n; i++) {
				result[i][i] = func.distance(x[i], x[i]);
				for (int j = i + 1; j < n; j++)
					result[j][i] = result[i][j] = func.distance(x[i], x[j]);
			}
	...
	public final double[][] calculateCovariances(final Kernel func,
		...
		for (int i = 0; i < n1; i++)
			for (int j = 0; j < n2; j++)
				result[i][j] = func.distance(x1[i], x2[j]);
		return result;
	}

	public final double[][] calculateCovarianceDerivatives(
			final Kernel func, final int hyperparamIndex) {
		final double[][] x = this.dataX;
		final int n = this.getSize();
