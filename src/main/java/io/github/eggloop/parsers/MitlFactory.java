...
import io.github.eggloop.expr.*;
import io.github.eggloop.mitl.*;
import io.github.eggloop.modelChecking.SignalFunction;
import io.github.eggloop.modelChecking.SignalFunctionType;
...
public class MitlFactory {
	...
	public MitlPropertiesList constructProperties(String text) {
		...
		MiTLParser.eval_return r = parser.eval();
	...
	public MiTL constructProperty(String text) {
		MiTLLexer lex = new MiTLLexer(new ANTLRStringStream(text));
		CommonTokenStream tokens = new CommonTokenStream(lex);
		MiTLParser parser = new MiTLParser(tokens);
		Tree ast = (Tree) parser.eval().getTree();
		return constructMiTL(ast);
	}
