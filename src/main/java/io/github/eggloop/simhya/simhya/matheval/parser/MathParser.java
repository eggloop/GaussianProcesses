...
package io.github.eggloop.simhya.simhya.matheval.parser;
...
public class MathParser implements MathParserConstants {
    ...
    final public Expression Start() throws ParseException, NumberFormatException, RuntimeException, ParseException {
        ...
        if (getToken(1).kind == IDENTIFIER && getToken(2).kind == LR && !evaluator.isFunction(getToken(1).image)) {
            ...
            try {
                evaluator.addExpressionVariable(t.image, exp);
            } catch (EvalException e) {
                {
                    if (true) throw new ParseException("Cannot define expression variable " + t.image + ":\u005cn" + e);
                }
            }
            {
                if (true) return null;
    ...
    final public void FunctionDef() throws ParseException, NumberFormatException, RuntimeException, ParseException {
        ...
        switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
            case IDENTIFIER:
                t = jj_consume_token(IDENTIFIER);
                try {
                    localVariables.addSymbol(t.image, 0.0);
                } catch (EvalException e) {
                    {
                        if (true)
                            throw new ParseException("Error in local parameter definition of function " + functionName +
                                    ": " + e.getMessage());
                    }
                }
                functionArity++;
