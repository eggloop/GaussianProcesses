 ...
 */
package io.github.eggloop.simhya.simhya.modelchecking.mtl.signal;
...
public class BooleanSignal {
    LinkedList<TimeInterval> signal;
    double initial_time;
    double final_time;
    ...
    public void addNextPositive(double t0next, double t1next, boolean lowerClosed, boolean upperClosed) {
        ...
        if (signal.isEmpty()) {
            if (t0next > this.initial_time || !lowerClosed) {
                signal.add(new TimeInterval(this.initial_time,t0next,false,true,!lowerClosed));
                signal.add(new TimeInterval(t0next,t1next,true,lowerClosed,upperClosed));
            } else
                signal.add(new TimeInterval(t0next,t1next,true,lowerClosed,upperClosed));
        } 
        else {
            ...
            if (t0next > T) {
                if (v)
                    signal.add(new TimeInterval(T,t0next,false,!c,!lowerClosed));
                else {
                ...
                if (v) {
                   if (t0next == T && !c && !lowerClosed) {
                      signal.add(new TimeInterval(t0next,t0next,false,true,true));
                      signal.add(new TimeInterval(t0next,t1next,true,lowerClosed,upperClosed));
                   } else {
...
//    }
    ...
    public void compact() {
        iterator = signal.iterator();
        TimeInterval next = null, current = (iterator.hasNext()?iterator.next():null);
        while (iterator.hasNext()) {
            next = iterator.next();
...
//    }
    ...
    public boolean getValueAt(double T) {
        if (!this.belongsTo(T))
            throw new RuntimeException("Cannot get value at time " + T + ": it does not belong to signal");
        TimeInterval I = this.getNext();
        while (I!=null) {
            if (I.contains(T))
                break;
...
//////        int n = signal.size();
//////        //ArrayList<PositiveInterval> overlapping = new ArrayList<PositiveInterval>();
//////        int i;
//////        for (i=0;i<n;i++) {
